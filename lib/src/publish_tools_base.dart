...
class PublishTools {
 ...
 A tap for the ${ptConfig.github.repoName} v${pubSpec.version} Dart package''',
          mode: FileMode.write);
  ...
  test do
    assert_match "obs_websocket v${pubSpec.version}", shell_output("${ptConfig.homebrew.binFolder}/${ptConfig.homebrew.executableName} version")
  end
end''';
      ...
      await runGit(['commit', '-m', 'v${pubSpec.version}']);
  ...
  static Future<void> _release() async {
    ...
    try {
      final release = await github.repositories
          .getReleaseByTagName(slug, 'v${pubSpec.version}');
    ...
    final createRelease = CreateRelease('v${pubSpec.version}');

    await github.repositories.createRelease(slug, createRelease);

    log('Release v${pubSpec.version} created.');
  }
